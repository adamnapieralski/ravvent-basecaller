...
class TrainJointBasecaller(tf.keras.Model):
    def __init__(self, units: int, output_text_processor, input_data_type: str, input_padding_value, teacher_forcing: bool = True, val_teacher_forcing: bool = False, use_tf_function: bool = True):
        super().__init__()
        # Build the encoder and decoder
    ...
    def _preprocess(self, raw_sequence, event_sequence, target_sequence):
        input_mask_raw = utils.input_mask(raw_sequence, self.input_padding_value)
        input_mask_event = utils.input_mask(event_sequence, self.input_padding_value)

        input_mask_joint = tf.concat((input_mask_raw, input_mask_event), axis=-1)
        ...
        return raw_sequence, event_sequence, input_mask_joint, target_tokens, target_mask

    def _train_step(self, data):
        raw_sequence, event_sequence, target_sequence = data

        raw_sequence, event_sequence, input_mask_joint, target_tokens, target_mask = self._preprocess(raw_sequence, event_sequence, target_sequence)

        max_target_length = tf.shape(target_tokens)[1]

        with tf.GradientTape() as tape:
            # Encode the input
            enc_output_raw, enc_state_raw = self.encoder_raw(raw_sequence)
            self.shape_checker(enc_output_raw, ('batch', 's', 'enc_units'))
            self.shape_checker(enc_state_raw, ('batch', 'enc_units'))

            enc_output_event, enc_state_event = self.encoder_event(event_sequence)

            enc_output_joint = tf.concat((enc_output_raw, enc_output_event), axis=1)
            enc_state_joint = tf.concat((enc_state_raw, enc_state_event), axis=1)


            # Initialize the decoder's state to the encoder's final state.
            # This only works if the encoder and decoder have the same number of
            # units.
            dec_state = enc_state_joint
            print(dec_state)
            loss = tf.constant(0.0)
            ...
            for t in tf.range(max_target_length-1):
                ...
                step_loss, dec_state, pred_tokens = self._loop_step(input_tokens, target_pred_tokens, input_mask_joint, enc_output_joint, dec_state, return_prediction_tokens=True)
                loss = loss + step_loss
    ...
    def _test_step(self, data):
        ...
        batch_size = tf.shape(raw_sequence)[0]
        done = tf.zeros([batch_size, 1], dtype=tf.bool)
        result_tokens = result_tokens.write(0, input_tokens)
